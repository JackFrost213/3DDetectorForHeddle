...
import detector.items.Canvas3D;
...
public class Panel3D extends JPanel implements GLEventListener, ComponentListener {
	...
	// the list of 3D items to be drawn
	protected Vector<Item3DImp2> _itemList2 = new Vector<Item3DImp2>();
	...
	private JPanel JMonkey3DPanel;
	
	private Canvas3D ImageSubstitute3D;
	 ...
	 * @param zdist the initial viewer z distance should be negative
	 */
	public Panel3D(CedView3DImp2 cedView, JPanel panel, float angleX, float angleY, float angleZ, float xDist, float yDist,
			float zDist, float bgRed, float bgGreen, float bgBlue) {
		this(angleX, angleY, angleZ, xDist, yDist, zDist, bgRed, bgGreen, bgBlue);
		remove(gljpanel);
		JMonkey3DPanel = new JPanel();
		JMonkey3DPanel.setLayout(null);
		// System.out.println("Canvas: " + panel.getComponent(0));
		try {
			JMonkey3DPanel.add(panel.getComponent(0), 0);
			cedView.setHasJMonkeyCanvas(true);
		} catch (Exception e) {
			System.out.println("Other panel already has canvas! Cannot be added again");
		}
		ImageSubstitute3D = new Canvas3D();
		try {
			ImageSubstitute3D.setSize(panel.getComponent(0).getSize());
		} catch (ArrayIndexOutOfBoundsException e) {
			ImageSubstitute3D.setSize(panel.getSize());
		}
		JMonkey3DPanel.add(ImageSubstitute3D);
		ImageSubstitute3D.setVisible(true);

		add(getJMonkey3DPanel(), BorderLayout.CENTER);
		this.addComponentListener(this);
		synchronized (cedView.getScene()) {
			createInitialItems(cedView.getScene());
		}
	}
...
//		System.err.println(Thread.currentThread().getName());
	...
	public void display(GLAutoDrawable drawable) {
		...
		// test tubes
		// Support3D.drawLine(drawable, 300f, 300f, 200f, -200f, -50f,
		// -50,Color.magenta, 1f);
...
//		gl.glClearColor(0.9412f, 1f, 1f, 1f); // set background (clear) color
	...
	/**
	 * Add an item to the JMonkey3D list
	 * 
	 * @param item the item to add.
	 */
	public void addItem(Item3DImp2 item) {
		if (item != null) {
			_itemList2.remove(item);
			_itemList2.add(item);
		}
	}
	...
	/**
	 * Create the initial items for 3D scene
	 */
	...
	public void componentResized(ComponentEvent e) {
		try {
			Canvas JMonkey3DCanvas = (Canvas) getJMonkey3DPanel().getComponent(0);
			int canvasWidth = (int) (getJMonkey3DPanel().getWidth() + getJMonkey3DPanel().getWidth() / 4);
			int canvasHeight = getJMonkey3DPanel().getHeight() + getJMonkey3DPanel().getHeight() / 4;
			Dimension dim = new Dimension(canvasWidth, canvasHeight);
			JMonkey3DCanvas.setSize(dim);
			ImageSubstitute3D.setSize(new Dimension(getJMonkey3DPanel().getWidth(), getJMonkey3DPanel().getHeight()));
		} catch (Exception ex) {
			// Ignore it
		}
	...
	public JPanel getJMonkey3DPanel() {
		return JMonkey3DPanel;
	}

	public void setJMonkey3DPanel(JPanel jMonkey3DPanel) {
		JMonkey3DPanel = jMonkey3DPanel;
	}
