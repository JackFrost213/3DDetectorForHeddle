package detector.items.JMonkey3DOverrides;
...
public class Geometry3DBatchFactory {

    private static final Logger logger = Logger.getLogger(Geometry3DBatchFactory.class.getName());
     ...
     */
    public static void mergeGeometries(Collection<Geometry3D> geometries, Mesh outMesh) {
    ...
    public static void makeLods(Collection<Geometry3D> geometries, Mesh outMesh) {
    ...
    public static List<Geometry3D> makeBatches(Collection<Geometry3D> geometries) {
     ...
     */
    public static List<Geometry3D> makeBatches(Collection<Geometry3D> geometries, boolean useLods) {
        ArrayList<Geometry3D> retVal = new ArrayList<>();
        HashMap<Material, List<Geometry3D>> matToGeom = new HashMap<>();

        for (Geometry3D geom : geometries) {
            List<Geometry3D> outList = matToGeom.get(geom.getMaterial());
            if (outList == null) {
            ...
            if (outList == null) {
                outList = new ArrayList<Geometry3D>();
        ...
        int batchNum = 0;
        for (Map.Entry<Material, List<Geometry3D>> entry : matToGeom.entrySet()) {
            Material mat = entry.getKey();
            List<Geometry3D> geomsForMat = entry.getValue();
            Mesh mesh = new Mesh();
            ...
            Geometry3D out = new Geometry3D("batch[" + (batchNum++) + "]", mesh);
            out.setMaterial(mat);
    ...
    public static void gatherGeoms(Spatial scene, List<Geometry3D> geoms) {
        if (scene instanceof Node) {
            Node node = (Node) scene;
            for (Spatial child : node.getChildren()) {
                gatherGeoms(child, geoms);
            }
        } else if (scene instanceof Geometry3D) {
            geoms.add((Geometry3D) scene);
        }
    }
    ...
    public static Node optimize(Node scene, boolean useLods) {
        ArrayList<Geometry3D> geoms = new ArrayList<>();

        gatherGeoms(scene, geoms);

        List<Geometry3D> batchedGeoms = makeBatches(geoms, useLods);
        for (Geometry geom : batchedGeoms) {
            scene.attachChild(geom);
        }

        for (Iterator<Geometry3D> it = geoms.iterator(); it.hasNext();) {
            Geometry3D geometry = it.next();
    ...
    public static void alignBuffers(Node n, AlignOption option) {
        List<Geometry3D> geoms = new ArrayList<>();
        gatherGeoms(n, geoms);
