...
import detector.items.views.CentralView3DImp2;
import detector.items.views.FTCalView3DImp2;
...
public class Ced extends BaseMDIApplication implements PropertyChangeListener, MagneticFieldChangeListener {
	...
	private ForwardView3DImp2 _forward3DViewImp2;
	private CentralView3DImp2 _central3DViewImp2;
	private FTCalView3DImp2 _ftCalView3DImp2;
	private ForwardView3D _forward3DView;
	...
	private void restoreDefaultViewLocations() {
		...
		if (_use3D) {
			_virtualView.moveTo(_forward3DViewImp2, 9, VirtualView.BOTTOMRIGHT);
			_virtualView.moveTo(_central3DViewImp2, 10, VirtualView.BOTTOMRIGHT);
			_virtualView.moveTo(_ftCalView3DImp2,9,VirtualView.TOPCENTER);
			_virtualView.moveTo(_forward3DView, 9, VirtualView.BOTTOMLEFT);
...
//			MainThread.getSingleton().useMainThread = true;
			ViewManager.getInstance().getViewMenu().addSeparator();
			MainDisplay3D mainDisplay = new MainDisplay3D();
			_forward3DViewImp2 = new ForwardView3DImp2(mainDisplay);
			_central3DViewImp2 = new CentralView3DImp2(mainDisplay);
			_ftCalView3DImp2 = new FTCalView3DImp2(mainDisplay);
			mainDisplay.setAllCedViewsAdded(true);
